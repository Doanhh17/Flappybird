...
const int g = 1;
const int birdjump = -12;
const int speedcol = 5;
const int dis_tance2cols = 250;
...
const int TITLE_HEIGH = 100;
const int SCORE_BOTTOM = heighwindow - 100;
...
SDL_Rect flappybird = { widthwindow / 8, heighwindow / 2, birdsize, birdsize };
...
SDL_Rect resumeRect = { widthwindow / 2 - BUTTON_WIDTH / 2, heighwindow / 2 - BUTTON_HEIGHT * 1.5 - BUTTON_DISTANCE, BUTTON_WIDTH, BUTTON_HEIGHT };
SDL_Rect restartRect = { widthwindow / 2 - BUTTON_WIDTH / 2, heighwindow / 2 - BUTTON_HEIGHT / 2, BUTTON_WIDTH, BUTTON_HEIGHT };
SDL_Rect pauseQuitRect = { widthwindow / 2 - BUTTON_WIDTH / 2, heighwindow / 2 + BUTTON_HEIGHT * 0.5 + BUTTON_DISTANCE, BUTTON_WIDTH, BUTTON_HEIGHT };
...
void update() {
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_QUIT) {
            ...
            if (e.key.keysym.sym == SDLK_p && !inMenu) {
                isPaused = !isPaused;
            }
            else if (e.key.keysym.sym == SDLK_SPACE && !inMenu && !isPaused) {
    ...
    if (!inMenu && !isPaused) {
        ...
        if (col_list.empty() || col_list.back().x + widthcol < widthwindow - 300) {
            col_list.push_back(uppercol(widthwindow, rand() % (heighwindow - dis_tance2cols - 50)));
        }
...
void RENDER() {
    ...
    if (inMenu) {
        // Tiêu đề game - căn giữa hoàn hảo
